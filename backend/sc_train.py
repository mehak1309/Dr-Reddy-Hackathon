...
class class1(object):
    ...
    def decode_sequences(self, inputs, targets, input_ctable, target_ctable,
        ...
        target_tokens  = [re.sub('[%s]' % self.EOS, '', token)
...
class trainer(object):
    ...
    def train_model(self, save_path):
        obj = class1()
        text = obj.read_text(self.data_path)
        vocab = obj.tokenize(text)
        vocab = list(filter(None, set(vocab)))

        # `maxlen` is the length of the longest word in the vocabulary
        # plus two SOS and EOS characters.
        maxlen = max([len(token) for token in vocab]) + 2
        train_encoder, train_decoder, train_target = obj.transform(
        ...
        text = obj.read_text(self.data_path)
        val_tokens = obj.tokenize(text)
        val_tokens = list(filter(None, val_tokens))

        val_maxlen = max([len(token) for token in val_tokens]) + 2
        val_encoder, val_decoder, val_target = obj.transform(
            val_tokens, maxlen, error_rate=self.error_rate, shuffle=False)
        ...
        train_steps = len(vocab) // self.train_batch_size
        val_steps = len(val_tokens) // self.val_batch_size

        # Compile the model.
        model, encoder_model, decoder_model = obj.seq2seq(
            self.hidden_size, nb_input_chars, nb_target_chars)
        # print(model.summary())
        ...
        for epoch in range(self.nb_epochs):
            ...
            train_encoder, train_decoder, train_target = obj.transform(
                vocab, maxlen, error_rate=self.error_rate, shuffle=True)
            
            train_encoder_batch = obj.batch(train_encoder, maxlen, input_ctable,
                                        self.train_batch_size, self.reverse)
            train_decoder_batch = obj.batch(train_decoder, maxlen, target_ctable,
                                        self.train_batch_size)
            train_target_batch  = obj.batch(train_target, maxlen, target_ctable,
                                        self.train_batch_size)    

            val_encoder_batch = obj.batch(val_encoder, maxlen, input_ctable,
                                        self.val_batch_size, self.reverse)
            val_decoder_batch = obj.batch(val_decoder, maxlen, target_ctable,
                                        self.val_batch_size)
            val_target_batch  = obj.batch(val_target, maxlen, target_ctable,
                                        self.val_batch_size)

            train_loader = obj.datagen(train_encoder_batch,
                                    train_decoder_batch, train_target_batch)
            val_loader = obj.datagen(val_encoder_batch,
